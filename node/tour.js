...
// mission tour
// on teste la distance avec le mur devant
// si suffisante on teste la distance à gauche
// si suffisante on avance
// si trop faible ou trop grande, on s'éloigne ou se rapproche du mur
// si la distance avec le mur devant est trop faible on tourne à gauche
...
function exec(etat_arduino) {
    ...
    console.log("exec tour2")

    let cmd = "" // commande Ã  envoyer au robot

    if (time == null) {
        console.log("erreur tour2 time = null")
    ...
    if (Date.now() > time + periode) { // on passe ici une fois par seconde (cas periode = 1000)
        ...
        if (count <= 0) {
        ...
        else if (etat_arduino.d[D_AVANT] > dist) { // distance suffisante, on se déplace pendant 100 ms
            if ( (etat_arduino.d[D_GAUCHE] > dist + 100 ) && (a_tourner == 0) && (sens != 1) ) {
            ...
            cmd = "[[t 600][mda 0][mga 200]]" // distance trop faible à l'avant, on tourne à droite
        }
